...
############ STORING RESULTS FOR RESPONSE RATIO ANALYSIS #########
...
### RESPONSE RATIO OF EACH CWM-TRAIT INDIVIDUALLY #############
...
{gg_cwm_wp_dynamics <- 
    ...
    geom_line(aes(color = RR_descriptor, group = year), linewidth = 0.5) +
    ...
    geom_hline(yintercept= 0, linetype = "dashed", color = p_CB, linewidth = 0.5) +
...
ggsave("results/Plots/protofinal/1.Warming_Effect_TRAITS.png", plot = gg_Warming_Effect, dpi = 300)
...
ggsave("results/Plots/protofinal/1.Results_TRAITS.png", plot = gg_Results_traits, dpi = 300)
