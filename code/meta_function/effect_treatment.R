...
effect_size_treatment <- function(data, variable){
...
effect <- data %>% 
  select(treatment, all_of(mean_variable), all_of(sd_variable)) %>% 
  distinct()

effect_c <- effect %>% 
  filter(treatment == "c") %>% 
  select(all_of(mean_variable), all_of(sd_variable)) %>% 
...
RR_treatment <- effect %>% 
  filter(treatment != "c") %>% 
  left_join(effect_c, by = "treatment")
############# soluionar esto
...
gg_delta_RR <- 
ggplot(RR_treatment, aes(x = treatment, y = delta_RR)) + 
...
gg_sigma_RR <- 
ggplot(RR_treatment, aes(x = treatment,  y = sigma_RR)) + 
