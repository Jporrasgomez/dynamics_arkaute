...
$$
\bar{B_{i_{j}}} = 1.96 \left[{\frac{\bar{H}}{2}}(\bar{A_{bi_{j}}} + \bar{A_{hi_{j}}})\right]^{2/3}
$$

where:\
- $\bar{B_ij}$is the estimated biomass per average individual,\
- $\bar{H}$ is the average maximum plant height,\
- $\bar{A_b}$ is the mean basal area,\
- $\bar{A_h}$ is the mean mid-height area.

The values 1.96 and $2/3$ are commonly used in allometric estimations. We performed sensitivity analysis for these coefficients and finally conveyed with the authors about using the same values.The model only fits to individuals between 5 and 200 cm, so all individuals beyond those thresholds have to be removed. This modification did not affected the pool of species of the experiment and it only meant the rejection of `r nrow(f_5_200)/nrow(flora_medium)*100`% datapoints.

------------------------------------------------------------------------

**3. Estimation of biomass per square meter (**$m^2$) and per species´

The biomass at species level ($B_j$) is calculated by multiplying $B_i$ by the total number of individuals ($\delta$) for the species $j$. 
...
-   **Stability** The stability of the of the imputed data was assessed by first, checking if the distribution of the imputed values followed the same distribution of the original data (Figures x and y). In the following where the and second, by calculating the coefficient of variation, which presented an aggregate mean for the ten possible values iterated by the model of `r round(mean(imput_stability_db$CV),2)` $\pm$ `r round(sd(imput_stability_db$CV), 2)`(Figure z)
...
**4. Removing outliers from biomass at species level ($B_j$) **
...
**5. Filling gaps of biomass measurements for samplings 0, 1 and 2.**
