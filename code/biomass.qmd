...
-   **Stability** The stability of the of the imputed data was assessed by first, checking if the distribution of the imputed values followed the same distribution of the original data (Figures x and y). In the following where the and second, by calculating the coefficient of variation, which presented an aggregate mean for the ten possible values iterated by the model of `r round(mean(imputed_db$CV),2)` $\pm$ `r round(sd(imputed_db$CV), 2)`(Figure z)
