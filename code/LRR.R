...
ab_rich_dynamics <- merge(ab_rich_dynamics0, radcoeff_df)   ## Algo pasa aquí. Se reduce la base de datos. REVISAR!

NAs <- anti_join(ab_rich_dynamics0, ab_rich_dynamics)

mean_sd_abrich_dynamics<- ab_rich_dynamics %>%
  group_by(treatment, sampling, sampling_date, date, month) %>%
...
# Response ratio ####
# Response ratio is calculated by Ln(mean(T)/mean(C))

abrich_LRR <- ab_rich_dynamics0 %>% 
  group_by(sampling, treatment, sampling_date, date) %>% 
  summarize(mean_abundance = mean(abundance, na.rm = T),
            mean_richness = mean(richness, na.rm = T))


abrich_LRR <- abrich_LRR %>%
  pivot_wider(
    names_from = treatment,              # Columns will be based on trait_name levels
    values_from = c(mean_abundance, mean_richness)              # Values will come from trait_mean
  )

abrich_LRR$LRR_abundance_w_c <- log(abrich_LRR$mean_abundance_w / abrich_LRR$mean_abundance_c)
abrich_LRR$LRR_abundance_p_c <- log(abrich_LRR$mean_abundance_p / abrich_LRR$mean_abundance_c)
abrich_LRR$LRR_abundance_wp_c <- log(abrich_LRR$mean_abundance_wp / abrich_LRR$mean_abundance_c)
abrich_LRR$LRR_abundance_wp_w <- log(abrich_LRR$mean_abundance_wp / abrich_LRR$mean_abundance_w)
abrich_LRR$LRR_abundance_wp_p <- log(abrich_LRR$mean_abundance_wp / abrich_LRR$mean_abundance_p)

abrich_LRR$LRR_richness_w_c <- log(abrich_LRR$mean_richness_w / abrich_LRR$mean_richness_c)
abrich_LRR$LRR_richness_p_c <- log(abrich_LRR$mean_richness_p / abrich_LRR$mean_richness_c)
abrich_LRR$LRR_richness_wp_c <- log(abrich_LRR$mean_richness_wp / abrich_LRR$mean_richness_c)
abrich_LRR$LRR_richness_wp_w <- log(abrich_LRR$mean_richness_wp / abrich_LRR$mean_richness_w)
abrich_LRR$LRR_richness_wp_p <- log(abrich_LRR$mean_richness_wp / abrich_LRR$mean_richness_p)


#Calcular variabilidad de LRR
...
# Data visualization ####
#Añadir linea entre 0 y 1 

types <- c("richness", "abundance", "sigma_log", "mu_log", "Y_zipf", "biomass")

# Dynamics
source("code/plots_functions_flora/plot_dynamics.R") 
...
#Trying to generate the big plot

abrich_ref_c <- RR_ref_c %>%
  select(c(plot, treatment, sampling_date, month, starts_with("logRR_")))

biomass_ref_c <- RR_biomass_ref_c %>%
  select(c(plot, treatment, sampling_date, month, logRR_biomass))

RR_big_raw <- full_join(abrich_ref_c, biomass_ref_c)

RR_long <- RR_big_raw %>%
  pivot_longer(
    cols = -c(plot, treatment, sampling_date),
    names_to = "logRR_variable",
...
ggplot(RR_long, aes( x = logRR_value, y = logRR_variable)) + 
  facet_grid(~ treatment, labeller = labeller(treatment = treatment_labs_dynamics)) +  
  geom_violin( aes(color = "black", fill = treatment)) + 
  stat_summary(fun = "mean", geom = "crossbar", width = 0.5, color = "black", size = 0.2) +
...
rr_abrich_summer <- abrich_ref_c %>%
  filter(month %in%c("Jun", "Jul", "Aug"))
rr_biomass_summer <- biomass_ref_c %>%
  filter(month %in%c("Jun", "Jul", "Aug"))
RR_summer <- full_join(rr_abrich_summer, rr_biomass_summer)
RR_long_summer <- RR_summer %>%
  pivot_longer(
    cols = -c(plot, treatment, sampling_date, month),
    names_to = "logRR_variable",
    values_to = "logRR_value"
  )

ggplot(RR_long_summer, aes( x = logRR_value, y = logRR_variable)) + 
  facet_grid(~ treatment, labeller = labeller(treatment = treatment_labs_dynamics)) +  
